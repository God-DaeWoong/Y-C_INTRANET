...
- **v0.32** (2026-01-23) - 첨부파일 미리보기 기능 추가 🆕

  ### 1. 결재 대기함 (approval-pending.html)

  **이미지/엑셀 미리보기 기능 추가**:
  - 첨부파일 목록에 "미리보기" 버튼 추가
  - 이미지 파일: 모달에서 원본 크기로 표시
  - 엑셀 파일: SheetJS 라이브러리로 파싱 후 HTML 테이블 렌더링
  - 미리보기 가능 파일: jpg, png, gif, xlsx, xls, csv

  ### 2. 내 문서함 (my-documents.html)

  **첨부파일 다운로드 및 미리보기 기능 추가**:
  - 문서 상세 조회 시 첨부파일 목록 표시 (기존에 없었음)
  - 각 파일별 다운로드 버튼 제공
  - 이미지/엑셀 파일 미리보기 지원
  - `getOriginalFileName()`: UUID 접두어 제거
  - `formatFileSize()`: 파일 크기 포맷팅

  ### 3. 문서 작성 (document-create.html)

  **엑셀 파일 업로드 전 미리보기 기능 추가**:
  - 첨부파일 목록에 "미리보기" 버튼 추가 (이미지/엑셀)
  - `previewFile()`: File 객체 기반 미리보기
  - `previewExcelFile()`: FileReader + SheetJS 조합
  - 이미지: `URL.createObjectURL()` 사용
  - 엑셀: ArrayBuffer → SheetJS 파싱 → HTML 테이블
  ...
  **미리보기 모달 CSS**:
  - `.preview-modal`: 전체 화면 오버레이 (z-index: 2000)
  - `.preview-modal-content`: 최대 90% 너비/높이
  - `.preview-excel-table`: 엑셀 데이터 테이블 스타일

  **파일 타입 판별 함수**:
  - `isImageFile()`: 이미지 확장자 판별
  - `isExcelFile()`: 엑셀 확장자 판별

---

- **v0.31** (2026-01-22) - 문서 작성 첨부파일 드래그&드롭 기능 추가

  ### 1. 드래그&드롭 업로드 영역 추가 (document-create.html)

  **변경 전**: 파일 선택 버튼으로만 업로드 가능
  **변경 후**: 드래그&드롭 영역 + 파일 선택 버튼 병행

  **새 UI 구성**:
  - 점선 테두리의 드롭존 영역
  - 📁 아이콘 + 가이드 텍스트
  - 드래그 시 시각적 피드백 (색상 변경, 살짝 확대)
  - "파일 선택" 버튼 유지 (폴백)

  ### 2. 이미지 파일 미리보기

  이미지 파일 첨부 시 썸네일 미리보기 표시:
  - `isImageFile()`: 이미지 타입 판별
  - `createPreviewUrl()`: `URL.createObjectURL()` 사용
  - 48x48 크기 썸네일 표시

  ### 3. 파일 타입별 아이콘

  `getFileIcon()` 함수로 파일 확장자별 아이콘 표시:
  - PDF: 📄
  - Word: 📝
  - Excel: 📊
  - PowerPoint: 📑
  ...
  **변경 후**: 카드 형태의 파일 항목
  ...
  | 파일 | 변경 내용 |
  |------|----------|
  | `document-create.html` | 드롭존 CSS 추가, HTML 영역 변경, 드래그 이벤트 핸들러 추가, updateFileList() 개선 |

  ### 6. 위원회 검토 결과

  - **대웅** (아키텍트): 기존 로직 유지하며 UI만 개선, 승인
  - **의섭** (DB): DB 변경 없음
  - **준영** (인프라): 파일 크기 제한(10MB) 기존 로직 유지
  - **현수** (개발): 작업 분해 완료, 구현 완료
  - **충원** (리스크): 기존 파일 선택 기능 폴백으로 유지
  - **은지** (UI/UX): 드래그&드롭은 직관적인 UX 개선
